...
public class LFunctionType : BObjectType<LFunction>
{
    ...
    public override LFunction Parse(Stream stream, BHeader header)
    {
        ...
        return new LFunction(header, s.Code, s.Locals.AsArray(), s.Constants.AsArray(), s.Upvalues, s.Functions.AsArray(), s.MaximumStackSize,
            s.LenUpvalues, s.LenParameter, s.VarArg);
    }

    protected virtual void ParseCode(
        Stream stream,
        BHeader header,
        LFunctionParseState s
    )
    {
        if (header.Debug)
        ...
        s.Length = header.Integer.Parse(stream, header)
            .AsInteger();
    ...
    protected virtual void ParseConstants(
        Stream stream,
        BHeader header,
        LFunctionParseState s
    )
    ...
    protected virtual void ParseDebug(
        Stream stream,
        BHeader header,
        LFunctionParseState s
    )
    {
        ...
        for (var i = 0; i < count; i++)
            s.Upvalues[i].Name = upvalNames[i]
                .DeRef();
    }

    protected virtual void ParseMain(
        Stream stream,
        BHeader header,
        LFunctionParseState s
    )
    {
        ...
        s.LineBegin = header.Integer.Parse(stream, header)
            .AsInteger();
        s.LineEnd = header.Integer.Parse(stream, header)
            .AsInteger();
    ...
    protected virtual void ParseUpvalues(
        Stream stream,
        BHeader header,
        LFunctionParseState s
    )
...
{
    protected override void ParseDebug(
        Stream stream,
        BHeader header,
        LFunctionParseState s
    )
    ...
    protected override void ParseUpvalues(
        Stream stream,
        BHeader header,
        LFunctionParseState s
    )
