using UnluacNET.Core.Decompile.Block;
...
{
    private static Stack<Branch.Branch> _mBackup;
    ...
    private List<Block.Block> blocks;
    private Block.Block outer;
    ...
    public Decompiler(LFunction function)
    {
        ...
        tForTarget = function.Header.LuaVersion.ForTarget;
    }

    public Code Code { get; }
    public Declaration[] DeclList { get; }
    ...
    private Block.Block EnclosingBlock(int line)
    {
    ...
    private Block.Block EnclosingBlock(Block.Block block)
    {
        //Assumes the outer block is first
        var outer = blocks[0];
        var enclosing = outer;
    ...
    private Block.Block EnclosingBreakableBlock(int line)
    {
        var outer = blocks[0];
        var enclosing = outer;
    ...
    private Block.Block EnclosingUnprotectedBlock(int line)
    {
        //Assumes the outer block is first
        var outer = blocks[0];
        var enclosing = outer;
    ...
    private Target.Target GetMoveIntoTargetTarget(int line, int previous)
    {
        switch (Code.Op(line))
        {
            case Op.Move:
    ...
    private Expression.Expression GetMoveIntoTargetValue(int line, int previous)
    {
        var a = Code.A(line);
        var b = Code.B(line);
        var c = Code.C(line);
    ...
    private OuterBlock HandleBranches(bool first)
    {
        ...
        if (!first)
        {
            ...
            var delete = new LinkedList<Block.Block>();
        ...
        var stack = new Stack<Branch.Branch>();
        ...
        for (var line = 1; line <= length; line++)
        {
                    ...
                    case Op.Jmp:
                    {
                        ...
                        else if (first || loopRemoved[line])
                        {
                            ...
                            else
                            {
                                ...
                                if (enclosing != null && enclosing.Breakable && Code.Op(enclosing.End) == Op.Jmp &&
            ...
            if (reduce)
            {
                ...
                var conditions = new Stack<Branch.Branch>();
                var backups = new Stack<Stack<Branch.Branch>>();
                ...
                do
                {
                    ...
                    else if (assignEnd - 3 >= 1 && Code.Op(assignEnd - 2) == Op.Loadbool && Code.C(assignEnd - 2) != 0 &&
                             Code.Op(assignEnd - 3) == Op.Jmp && Code.SBx(assignEnd - 3) == 2)
                    {
                    ...
                    else if (assignEnd - 2 >= 1 && Code.Op(assignEnd - 1) == Op.Loadbool && Code.C(assignEnd - 1) != 0 &&
                             Code.Op(assignEnd - 2) == Op.Jmp && Code.SBx(assignEnd - 2) == 2)
                    {
                        if (peekNode is TestNode)
                        {
                            isAssignNode = true;
                    ...
                    {
                        if (Code.Op(assignEnd) == Op.Loadbool && Code.C(assignEnd) != 0 && Code.Op(assignEnd - 1) == Op.Jmp &&
                        ...
                        else if (r.IsLocal(GetAssignment(assignEnd - 1), assignEnd - 1) && assignEnd > peekNode.Line)
                        {
                    ...
                    if (!compareCorrect && assignEnd - 1 == peekNode.Begin && Code.Op(peekNode.Begin) == Op.Loadbool && Code.C(peekNode.Begin) != 0)
                    {
                        _mBackup = null;
                    ...
                    {
                        _mBackup = new Stack<Branch.Branch>();
                ...
                do
                {
                    ...
                    if (cond.IsSet)
                    {
                        ...
                        if (Code.Op(cond.Begin) == Op.Jmp && Code.SBx(cond.Begin) == 2 && Code.Op(cond.Begin + 1) == Op.Loadbool &&
                    ...
                    else if (hasTail)
                    {
                        ...
                        if (tail > cond.End || (tail == cond.End && !isEndCondJump))
                        {
                            ...
                            var isBreakableLoopEnd = Function.Header.LuaVersion.IsBreakableLoopEnd(op);
        ...
        var newBlocks = new List<Block.Block>();
    ...
    private LinkedList<Operation.Operation> ProcessLine(int line)
    {
        var operations = new LinkedList<Operation.Operation>();
        ...
        switch (Code.Op(line))
        {
            ...
            case Op.Loadnil:
            {
                var maximum = Function.Header.LuaVersion.UsesOldLoadNilEncoding ? b : a + b;

                while (a <= maximum)
                {
                    operations.AddLast(new RegisterSet(line, a, Expression.Expression.NIL));
                    a++;
                }
            }
                break;
            ...
            case Op.Gettabup:
            {
                var expr = b == 0 && (c & 0x100) != 0
                    ? F.GetGlobalExpression(c & 0xFF)
                    : new TableReference(upvalues.GetExpression(b), r.GetKExpression(c, line)) as Expression.Expression;
            ...
            case Op.Settabup:
                if (a == 0 && (b & 0x100) != 0)
                    ...
                    operations.AddLast(
                        new TableSet(line, upvalues.GetExpression(a), r.GetKExpression(b, line), r.GetKExpression(c, line), true, line));
                break;
            case Op.Setglobal:
            ...
            case Op.Add:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakeAdd(r.GetKExpression(b, line), r.GetKExpression(c, line))));
                break;
            case Op.Sub:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakeSub(r.GetKExpression(b, line), r.GetKExpression(c, line))));
                break;
            case Op.Mul:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakeMul(r.GetKExpression(b, line), r.GetKExpression(c, line))));
                break;
            case Op.Div:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakeDiv(r.GetKExpression(b, line), r.GetKExpression(c, line))));
                break;
            case Op.Mod:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakeMod(r.GetKExpression(b, line), r.GetKExpression(c, line))));
                break;
            case Op.Pow:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakePow(r.GetKExpression(b, line), r.GetKExpression(c, line))));
                break;
            case Op.Unm:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakeUnm(r.GetExpression(b, line))));
                break;
            case Op.Not:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakeNot(r.GetExpression(b, line))));
                break;
            case Op.Len:
                operations.AddLast(new RegisterSet(line, a, Expression.Expression.MakeLen(r.GetExpression(b, line))));
                break;
            ...
            case Op.Concat:
            {
                ...
                while (c-- > b)
                    value = Expression.Expression.MakeConcat(r.GetExpression(c, line), value);
            ...
            case Op.Call:
            {
                ...
                var arguments = new Expression.Expression[b - 1];
            ...
            case Op.Return:
            {
                ...
                var values = new Expression.Expression[b - 1];
            ...
            case Op.Setlist:
            {
                ...
                for (var i = 1; i <= b; i++)
                    operations.AddLast(new TableSet(line, table, new ConstantExpression(new Constant((c - 1) * 50 + i), -1),
            ...
            case Op.Closure:
            {
                ...
                if (Function.Header.LuaVersion.UsesInlineUpvalueDeclaritions)
    ...
    private Assignment ProcessOperation(
    ...
    )
    {
        ...
        if (stmt != null)
        {
            ...
            if (assign != null)
            {
                ...
                if (wasMultiple && !assign.GetFirstValue()
                        .IsMultiple)
                    block.AddStatement(stmt);
            }
        }
    ...
    {
        var blockIndex = 1;
        var blockStack = new Stack<Block.Block>();
        ...
        {
            Operation.Operation blockHandler = null;

            while (blockStack.Peek()
                       .End <= line)
            {
                blockHandler = blockStack.Pop()
                    .Process(this);
            ...
            if (blockHandler == null)
            {
                ...
                else
                {
                    ...
                    if (assign != null && assign.GetFirstValue()
                            .IsMultiple)
                        block.AddStatement(assign);
                }
            }
    ...
    public Branch.Branch PopCompareSetCondition(
    ...
    public Branch.Branch PopCondition(Stack<Branch.Branch> stack)
    {
        var branch = stack.Pop();

        if (_mBackup != null)
            _mBackup.Push(branch);
        ...
        while (stack.Count > 0)
        {
            ...
            if (next.End == begin)
                branch = new OrBranch(PopCondition(stack)
                    .Invert(), branch);
    ...
    public Branch.Branch PopSetCondition(
        Stack<Branch.Branch> stack,
        int assignEnd,
        int target
    )
    ...
    private Branch.Branch PopSetConditionInternal(
        Stack<Branch.Branch> stack,
        bool invert,
        int assignEnd,
        int target
