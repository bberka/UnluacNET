...
public class ClosureExpression : Expression
{
    ...
    public override int ClosureUpvalueLine => m_upvalueLine;
    ...
    public override void PrintClosure(Output output, Target.Target name)
    {
        var d = new Decompiler(m_function);
        output.Print("function ");

        if (m_function.NumParams >= 1 && d.DeclList[0]
                .Name.Equals("self") && name is TableTarget)
        {
